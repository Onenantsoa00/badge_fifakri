...
membresRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const styleId = req.body.matricule_style_id
        ? Number(req.body.matricule_style_id)
        : null;
      ...
      for (let idx = 0; idx < parsed.rows.length; idx++) {
        ...
        if (!r.nom || !r.prenoms || !r.eglizy || !r.tokim_panompoana) {
          errors.push({
            line,
            error: "Ligne incomplète (nom, prénoms, église, tokim requis)",
          });
          continue;
        }
        ...
        try {
          ...
          if (e.code === "23505") {
            errors.push({
              line,
              error: `Matricule déjà utilisé : ${matricule}`,
            });
          } else {
            errors.push({ line, error: e.message || String(e) });
          }
...
membresRouter.delete("/", async (_req, res, next) => {
  try {
    await query("DELETE FROM membres");
    res.status(204).send();
  } catch (e) {
    next(e);
  }
...
membresRouter.patch("/:id", async (req, res, next) => {
  try {
    ...
    if (e.code === "23505")
      res.status(409).json({ error: "Matricule déjà utilisé" });
    else next(e);
  }
});
...
membresRouter.delete("/:id", async (req, res, next) => {
  try {
    const { rowCount } = await query(
      `DELETE FROM membres WHERE id = $1::uuid`,
      [req.params.id],
    );
    if (!rowCount) {
